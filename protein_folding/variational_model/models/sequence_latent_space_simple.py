...
def DecoderTransformLayer(num_blocks, num_heads, key_dim,
    num_dnn_layers, ideal_sequence_size, inputs, inputs_mask, num_transformers, channel_size):
  x = inputs
  ...
  for i in range(num_transformers):
    ...
    x = AttentionLayer(
        num_blocks, num_heads, key_dim, ideal_sequence_size, channel_size, x, inputs_mask)
    ...
    x = FeedForwardLayer(num_dnn_layers, ideal_sequence_size, channel_size,
        tf_keras.layers.Dense(channel_size)(
            tf_keras.layers.concatenate([x, conv_inputs])))
  return x

def _DecoderTransformer(
    base_features, num_transformers, atom_mask, num_blocks, channel_size):
