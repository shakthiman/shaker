...
def AttentionLayer(num_blocks, num_heads, key_dim, timesteps, embedding_size, inputs, inputs_mask):
  ...
  return tf.keras.layers.LayerNormalization(name='layer-normalization-1-'+str(random.randint(0, 10000)),
                                            axis=[1,2])(
      tf.keras.layers.Add()([
        inputs,
...
def FeedForwardLayer(num_layers, ideal_sequence_size, output_size, inputs):
  ...
  added_val = tf.keras.layers.Add()([inputs, t])
  added_val = tf.ensure_shape(added_val, [_BATCH_SIZE, ideal_sequence_size, output_size])
  return tf.keras.layers.LayerNormalization(name='layer-normalization-2-'+str(random.randint(0, 10000)),
                                            axis=[1,2])(
      added_val)
...
def LocalRotationModel():
  local_normalized_coordinates_mean_removed = tf.keras.Input(
  ...
  num_local_atoms = tf.keras.Input(shape=[_NUM_PEPTIDES, _ATOMS_PER_SEQUENCE//_LOCAL_ATOMS_SIZE, 1],
                                   name='num_local_atoms')
  local_predicted_coordinates_mean_removed = tf.keras.Input(
      shape=[_NUM_PEPTIDES, _ATOMS_PER_SEQUENCE//_LOCAL_ATOMS_SIZE, _LOCAL_ATOMS_SIZE, 3],
