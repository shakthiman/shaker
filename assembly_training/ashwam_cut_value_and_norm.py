...
def main ():
  ...
  with strategy.scope():
    variational_model = sequence_latent_space_with_hierarchical_encoder.MODEL_FOR_TRAINING(
        v, config)
    optimizer = tf.keras.optimizers.Adam(clipnorm=5e5)
  ...
  model_trainer.Train(
    ...
    optimizer=optimizer,
    save_frequency=1000,
    write_target='gs://variational_shaker_models/assembly_based_model_prod_ashwam_cut_value_and_norm_fixed',
    tensorboard_target='gs://variational_shaker_models/tensorboard/assembly_based_model_prod_ashwam_cut_value_and_norm_fixed',
    checkpoint_directory='gs://variational_shaker_models/checkpoints/assembly_based_model_prod_ashwam_cut_value_and_norm_fixed',
    strategy=strategy,
