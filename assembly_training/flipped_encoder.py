...
def BetaAnneal(step):
  max_val_for_period = tf.math.minimum(10.0, 0.5 + 0.1*tf.cast(
...
def main ():
  ...
  config = {'scale_coordinates': False,
            'flip_encoder_processing': True}
  ...
  model_trainer.Train(
    ...
    optimizer=optimizer,
    save_frequency=1000,
    write_target='gs://variational_shaker_models/assembly_based_model_prod_leham',
    tensorboard_target='gs://variational_shaker_models/tensorboard/assembly_based_model_prod_leham',
    checkpoint_directory='gs://variational_shaker_models/checkpoints/assembly_based_model_prod_leham',
    strategy=strategy,
