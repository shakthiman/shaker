...
def BetaAnneal(step):
  return tf.math.minimum(10.0, tf.cast(step%10000, tf.float32)*2e-3)
...
def main ():
  ...
  model_trainer.Train(
    ...
    optimizer=optimizer,
    save_frequency=1000,
    write_target='gs://variational_shaker_models/assembly_based_model_prod_hail_mary',
    tensorboard_target='gs://variational_shaker_models/tensorboard/assembly_based_model_prod_hail_mary',
    checkpoint_directory='gs://variational_shaker_models/checkpoints/assembly_based_model_prod_hail_mary',
    strategy=strategy,
